...
export default function FuelMonitoring() {
  ...
  const { data: history = [] } = useQuery({
    ...
    queryFn: () => api.getFuelHistory(Number(selectedVehicle), histFrom, histTo),
    enabled: !!selectedVehicle && !!histFrom && !!histTo,
  });

  const { data: consumption } = useQuery({
    queryKey: ["fuel-consumption", selectedVehicle, histFrom, histTo],
    queryFn: () => api.getFuelConsumption(Number(selectedVehicle), histFrom, histTo),
    enabled: !!selectedVehicle && !!histFrom && !!histTo,
  });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {mergedFuel.map((f) => {
          const pct = f.fuel_tank_capacity > 0 ? (f.fuel_level / f.fuel_tank_capacity) * 100 : f.fuel_level;
          return (
            <Card key={f.vehicle_id}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-lg font-bold">{f.fuel_level.toFixed(1)}L</span>
                </div>
                <Progress value={Math.min(pct, 100)} className="h-2" />
                <p className="text-xs text-muted-foreground mt-1">
                  {pct.toFixed(0)}% • {f.fuel_tank_capacity}L capacity
                </p>
              </CardContent>
            </Card>
          );
        ...
        <CardContent className="space-y-4">
          ...
          {consumption && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Min Fuel", value: `${consumption.min_fuel.toFixed(1)}L`, icon: TrendingDown, color: "text-red-500" },
                { label: "Max Fuel", value: `${consumption.max_fuel.toFixed(1)}L`, icon: TrendingUp, color: "text-emerald-500" },
                { label: "Avg Fuel", value: `${consumption.avg_fuel.toFixed(1)}L`, icon: Droplets, color: "text-blue-500" },
                { label: "Consumed", value: `${consumption.fuel_consumed.toFixed(1)}L`, icon: Fuel, color: "text-amber-500" },
              ].map((s) => (
            ...
            <div className="h-[300px]">
              <ResponsiveContainer>
                <LineChart data={chartData}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="fuel" stroke="hsl(174, 62%, 38%)" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
